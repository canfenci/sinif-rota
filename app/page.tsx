...
export default function Home() {
  ...
  return <main className={`app-shell ${view === "quick" && statuses ? "quick-open" : ""}`}>
    ...
    {view === "student" && student && <StudentView student={student} schoolClass={schoolClass} onBack={() => navigate("class")} />}
    ...
    {view !== "quick" && view !== "student" && view !== "import" && (
      <nav className="bottom-nav" aria-label="Ana menü">
        <button className={view === "home" ? "nav-active" : ""} onClick={() => navigate("home")}>Ana Sayfa</button>
        <button className={view === "classes" || view === "class" ? "nav-active" : ""} onClick={() => navigate("classes")}>Sınıflar</button>
        <button className={view === "reports" ? "nav-active" : ""} onClick={() => navigate("reports")}>Raporlar</button>
        <button className={view === "plan" ? "nav-active" : ""} onClick={() => navigate("plan")}>Yıllık Plan</button>
      </nav>
    )}
...
function StudentView({ student, schoolClass, onBack }: { student: Student; schoolClass: SchoolClass; onBack: () => void }) {
  return (
    <>
      <AppHeader eyebrow={`${schoolClass.name} · ${student.number} NUMARA`} title={student.name} back={onBack} />
      <section className="rule-note">
        <strong>Değerlendirmeler ve Raporlar</strong>
        <p>
          Detaylı değerlendirmeler ve raporlar için Raporlar bölümünü kullanın.
        </p>
      </section>
    </>
  );
